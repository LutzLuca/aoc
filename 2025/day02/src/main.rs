...
fn part_1(ranges: &[RangeInclusive<usize>]) -> usize {
    ranges.iter().fold(0usize, |acc, range| {
        acc + range.clone().filter(|&id| is_invalid_id_1(id)).sum::<usize>()
    })
}

fn part_2(ranges: &[RangeInclusive<usize>]) -> usize {
    ranges.iter().fold(0usize, |acc, range| {
        acc + range.clone().filter(|&id| is_invalid_id_2(id)).sum::<usize>()
    })
}
...
fn is_invalid_id_1(id: usize) -> bool {
    ...
    for group_offset in 0..group_size {
        let curr_idx = group_offset;
        let prev_idx = group_size + group_offset;
...
fn is_invalid_id_2(id: usize) -> bool {
    ...
    let max_group = digits / 2;

    'outer: for group_size in 1..=max_group {
